...
def generate_seedance_prompts(
    ...
    if not keyword_segments:
        return {"prompts": [], "updated_at": datetime.now(timezone.utc).isoformat()}

    prompt_inputs = _build_prompt_segment_inputs(keyword_segments)

    prompt = f"""
SYSTEM:
You create JSON-only video prompts for Seedance Pro v1.

CONTEXT:
- These prompts are for short vertical b-roll inserts on top of a talking-head video.
- The generator model is Seedance Pro v1.
- Each generated clip must be maximum 5 seconds.
...
TASK:
For each keyword segment, create a structured JSON prompt suitable for Seedance Pro v1.
...
- Write the creative as a 5-second storyboard for Seedance Pro v1, but the camera language should stay user-shot and intimate.
- timing inside prompt_json must start at 0.0s and cover at most 5.0 seconds total.
...
{{
  "prompts": [
    {{
      ...
      "prompt_json": {{
        "global_logic": "Authentic vertical iPhone travel footage, user-shot feel, portrait-safe framing, natural light, realistic handheld motion, full storyboard duration 5.0 seconds maximum.",
        "scene_sequencing": [
          {{
            "shot_id": 1,
            "timing": "0.0s - 5.0s",
...
"""
    ...
    try:
        ...
        for segment in keyword_segments:
            use_ready_asset = segment.get("asset_type") == "product_video"
            duration = min(max(float(segment.get("slot_end", 0)) - float(segment.get("slot_start", 0)), MIN_BROLL_SEGMENT_SECONDS), 5.0)
            must_show = segment.get("visual_intent") or segment.get("phrase") or segment.get("keyword")
            scene_sequencing = [
                    {
                        "shot_id": 1,
            ...
            fallback.append({
                ...
                "prompt_json": None if use_ready_asset else {
                    "global_logic": "Authentic vertical iPhone travel footage, user-shot feel, natural light, portrait-safe framing, full storyboard duration 5.0 seconds maximum.",
